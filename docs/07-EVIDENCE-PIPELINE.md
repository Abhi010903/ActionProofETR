...
### 2.1. Independent ABI Decoder (`src/analysis/abiDecoder.ts`)
...
* **Standard Interface Support:**
  ...
  * **Uniswap V2 / V3:** `swapExactTokensForTokens`, `exactInputSingle`, `multicall`.
  * **Multicall3:** `aggregate3`, `aggregate`.
...
4. Emits individual `DecodedSubcall` objects for each internal action, enabling policy rules to inspect sub-operations independently.

### 2.3. Contract Identity & Sourcify Adapter (`src/evidence/sourcifyAdapter.ts`)
...
### 2.4. Clear Signing & ERC-7730 Adapter (`src/evidence/erc7730Adapter.ts`)
...
* Returns structured intent descriptors that feed directly into intent matching.

### 2.5. Simulation Adapter & Honest Provenance (`src/evidence/simulationAdapter.ts`)
...
* **Explicit Provenance Annotations:** ActionProof never pretends that fixture simulations are live network calls. Every simulation result includes a `provenance` tag:
  * `LIVE_EVM`: Executed against an active fork or JSON-RPC node via `eth_call` / debug tracing.
  * `LOCAL_FIXTURE`: Executed against deterministic local simulation fixtures for hermetic testing and demonstration.
  * `UNAVAILABLE`: Simulation provider not configured or network unreachable.
