...
* **ActionProof Treatment:** 
  ...
  3. Generates a deterministic, SHA-256 canonical commitment hash:
     $$\text{Commitment} = \text{SHA256}(\text{CanonicalSerialization}(\text{snapshot}))$$
...
* **ActionProof Treatment:**
  ...
  - **Recursive Multicall Inspection:** Recursively unpacks nested subcalls (e.g. `Multicall3.aggregate3`, `SwapRouter02.multicall`) to uncover hidden transfer or approval operations.
  - **Contract Identity Verification:** Checks contract bytecode and metadata via Sourcify/verification registries.
  - **ERC-7730 Intent Descriptors:** Maps contract parameters to human-readable intent schemas.
  - **Execution Simulation:** Analyzes predicted state changes, balance deltas, and asset transfers.
...
* **ActionProof Treatment:**
  - Before forwarding, the pre-forward barrier takes the exact payload about to be sent.
  - Re-canonicalizes and re-hashes the payload:
     $$\text{ForwardCommitment} = \text{SHA256}(\text{CanonicalSerialization}(\text{forwardPayload}))$$
  - Re-checks: $\text{ForwardCommitment} \equiv \text{VerifiedCommitment}$.
  - If any in-memory mutation, prototype pollution, or reference tamper occurred, ActionProof throws `COMMITMENT_MISMATCH` and terminates forwarding.
